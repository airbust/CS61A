...
def replace_leaf(t, old, new):
    ...
    for branch in branches(t):
        te.append(replace_leaf(branch, old, new))
    return tree(label(t), te)
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pw):
        nonlocal balance, password, attempt, locked
        if locked:
            return "Your account is locked. Attempts: " + repr(attempt)
        elif pw == password:
            if amount > balance:
                return 'Insufficient funds'
            balance -= amount
...
def quadratic(x, a, b, c):
    ...
    def f(x):
        return a*x*x + b*x + c
    t1, t2 = f(lower_bound(x)), f(upper_bound(x))
    minx = -b/(2*a)
    if a == 0:
        return interval(min(t1, t2), max(t1, t2))
    elif a > 0:
        if minx >= lower_bound(x) and minx <= upper_bound(x):
            return interval(f(minx), max(t1, t2))
        else:
            return interval(min(t1, t2), max(t1, t2))
    else:
        if minx >= lower_bound(x) and minx <= upper_bound(x):
            return interval(min(t1, t2), f(minx))
        else:
            return interval(min(t1, t2), max(t1, t2))
