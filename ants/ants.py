...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.is_true_queen:
            ...
            while place:
                if place.ant:
                    if isinstance(place.ant, BodyguardAnt) and place.ant.ant and place.ant.ant not in self.have_doubled:
                        place.ant.ant.damage *= 2
                        self.have_doubled.append(place.ant.ant)
                    if place.ant not in self.have_doubled:
                        place.ant.damage *= 2
                        self.have_doubled.append(place.ant)
                place = place.exit
        # END Problem 13
...
def make_slow(action):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    return lambda colony: None if colony.time % 2 != 0 else action(colony)
    # END Problem EC
